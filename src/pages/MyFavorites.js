...
const MyFavorites = () => {
  ...
  const getAds = async () => {
    ...
    const q = query(favRef, where('users', 'array-contains', auth.currentUser.uid))
    const docsSnap = await getDocs(q)

    let promises = []
    docsSnap.forEach(async doc => {
      //  get all the ads marked as favorite by currently logged in user.
